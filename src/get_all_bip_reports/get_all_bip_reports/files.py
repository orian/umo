...
filename_re_ = re.compile(r'attachment; ?filename="(.*)"')

def check_content_disposition(response):
    # Example 'Content-Disposition: attachment; filename="Zal_10a.pdf"'
    # another 'Content-Disposition:attachment; filename="Zal_1.pdf"'
    f = filename_re_.findall(response.headers.get('Content-Disposition', ''))
    return f[0] if len(f) else None

class FilesPipeline2(MediaPipeline):
    ...
    def __init__(self, store_uri, download_func=None):
        ...
        super(FilesPipeline2, self).__init__(download_func=download_func)
    ...
    def media_downloaded(self, response, request, info):
        ...
        filename = check_content_disposition(response)
        ret = {'url': request.url, 'path': path, 'checksum': checksum}
        if filename:
            ret['filename'] = filename
        return ret
